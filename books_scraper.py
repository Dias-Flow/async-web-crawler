...
class BooksScraper:
    ...
    async def _discover_book_urls(self) -> list[str]:
        ...
        while current_url and len(all_book_urls) < self.max_books:
            ...
            result = await self._crawler._fetch_url_internal(current_url)
            if not result.success:
                logger.warning(
                    "Failed to fetch listing page: %s — %s",
                    current_url, result.error,
                )
                break
    ...
    async def _scrape_book(self, url: str) -> Optional[Book]:
        ...
        result = await self._crawler._fetch_url_internal(url)
        ...
        book = self._detail_parser.parse(result.content or "", url)
